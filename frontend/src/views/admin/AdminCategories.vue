...
import { batchDeleteAdminCategories, batchUpdateAdminCategoryStatus, createAdminCategory, deleteAdminCategory, listAdminCategories, updateAdminCategory } from '@/services/admin'
...
function handleSelectionChange(keys) {
  selectedCategoryIds.value = keys || []
}
...
function handleBatchStatus(status) {
  ...
  confirmAction({
    title: '批量更新分类状态',
    content: `将 ${selectedCategoryIds.value.length} 个分类更新为${status === 'active' ? '启用' : '隐藏'}。`,
    okText: '确认',
    async onOk() {
      await runAction(() => batchUpdateAdminCategoryStatus(selectedCategoryIds.value, status), {
        successMessage: '分类状态已批量更新',
        errorMessage: '批量更新失败',
        onSuccess: () => {
          clearSelection()
...
function handleBatchDelete() {
  ...
  confirmAction({
    title: '批量删除分类',
    content: `将 ${selectedCategoryIds.value.length} 个分类批量删除。该操作仅处理分类记录，不会修改文章正文内容。`,
    okText: '确认删除',
    okType: 'danger',
    async onOk() {
      await runAction(() => batchDeleteAdminCategories(selectedCategoryIds.value), {
        successMessage: '分类已批量删除',
        errorMessage: '批量删除失败',
        onSuccess: () => {
          clearSelection()
          tableRef.value?.refresh()
