            ...
            >
              ...
              >
                <button
                  ...
                  :key="item.route"
                  type="button"
                  @click="handleQuickActionClick(item.route)"
                >
    ...
    >
      <div class="quick-config">
        <section class="quick-config__pane">
          <header>
            <strong>首页展示</strong>
            <span>{{ draftQuickRoutes.length }} 项</span>
          </header>
          <div
...
import {
  ...
  MonitorOutlined, SettingOutlined
...
const availableQuickActions = computed(() => {
  const flat = flattenMenus(authStore.managementMenus || [])
...
function flattenMenus(items = []) {
  ...
  for (const item of items) {
    ...
    if (item.enabled !== false && !item.hidden) {
      result.push(item)
    }
  }
  return result
...
function handleQuickActionClick(route) {
  if (quickPointerMoved.value) return
  router.push(route)
}
...
async function saveQuickConfig() {
  const routes = [...draftQuickRoutes.value]
  const savedRoutes = await updateQuickActions(routes)
  quickRoutes.value = savedRoutes
  authStore.user = {
    ...authStore.user,
    quickActions: savedRoutes
  }
  quickConfigOpen.value = false
}
...
function handleDraftDrop(index) {
