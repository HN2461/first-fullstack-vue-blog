...
import { batchReviewAdminComments, listAdminComments, reviewAdminComment } from '@/services/admin'
...
function batchReview(action) {
  ...
  confirmAction({
    title: `确认批量${actionLabelMap[action]}评论`,
    content: `将 ${selectedCommentIds.value.length} 条评论批量${actionLabelMap[action]}。`,
    okText: `确认${actionLabelMap[action]}`,
    okType: action === 'hide' ? 'danger' : 'primary',
    async onOk() {
      await runAction(() => batchReviewAdminComments(selectedCommentIds.value, action), {
        successMessage: '评论已批量处理',
        errorMessage: '批量处理失败',
        onSuccess: () => {
          clearSelection()
