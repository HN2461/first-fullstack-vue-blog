...
function handleBatchStatus(status) {
  ...
  confirmAction({
    title: '批量更新标签状态',
    content: `将 ${selectedTagIds.value.length} 个标签更新为${status === 'active' ? '启用' : '禁用'}。`,
    okText: '确认',
    async onOk() {
      await runAction(() => batchUpdateAdminTagStatus(selectedTagIds.value, status), {
        successMessage: '标签状态已批量更新',
        errorMessage: '批量更新失败',
        onSuccess: () => {
          clearSelection()
...
function handleBatchDelete() {
  ...
  confirmAction({
    title: '批量删除标签',
    content: `将 ${selectedTagIds.value.length} 个标签批量删除。该操作不会修改文章正文内容。`,
    okText: '确认删除',
    okType: 'danger',
    async onOk() {
      await runAction(() => batchDeleteAdminTags(selectedTagIds.value), {
        successMessage: '标签已批量删除',
        errorMessage: '批量删除失败',
        onSuccess: () => {
          clearSelection()
          tableRef.value?.refresh()
