    ...
    <div class="media-guide">
      <template v-if="topic === 'inventory'">
        ...
        <section class="media-guide__section">
          <h3>扫描范围</h3>
          <p>扫描上传存储目录中的文件，并排除已经存在媒体库记录的资源。页面只展示相对路径，不展示服务器绝对目录。</p>
        </section>

        <section class="media-guide__section">
          <h3>来源标签</h3>
          <dl class="media-guide__definitions">
            <div><dt>用户头像目录</dt><dd>由账号资料独立管理，不能登记为普通媒体资产。</dd></div>
            <div><dt>媒体上传目录</dt><dd>通过后台上传的常规位置，通常应存在对应的媒体库记录。</dd></div>
            <div><dt>测试目录</dt><dd>用于识别历史测试文件；确认未被引用后才可通过清理操作删除。</dd></div>
            <div><dt>上传目录</dt><dd>上传根目录下的历史、脚本或业务直写文件，需要结合引用状态判断用途。</dd></div>
          </dl>
        </section>
        ...
        <section class="media-guide__section">
          ...
          <ul>
            ...
            <li>清理只面向疑似测试且未登记的文件；存在引用或头像目录资源会被后端再次拦截。</li>
          </ul>
        </section>
      </template>
      ...
      <template v-else>
        <section class="media-guide__lead">
          ...
          <p>系统分类由业务流程写入，普通分类由管理员维护；两类分类的可编辑边界不同。</p>
        </section>
        ...
        <section class="media-guide__section">
          <h3>自定义分类与待归档</h3>
          <ul>
            <li>管理员可以新建、编辑或删除无资源引用的自定义分类，用于项目截图、课程资料等业务归档。</li>
