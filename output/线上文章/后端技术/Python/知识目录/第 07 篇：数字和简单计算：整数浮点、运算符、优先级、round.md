---
title: "第 7 篇：数字和简单计算：整数浮点、运算符、优先级、round"
...
originalSlug: "python-zero-numbers-and-operators"
...
Python 还支持复数类型 `complex`，例如 `3 + 4j`。它主要用于科学计算、信号处理等场景，普通 Web 后端和自动化脚本很少直接使用，本篇先不展开。这里的“两种”指最常用的 `int` 和 `float`，不是说 Python 只有两种数字类型。
...
新手阶段先掌握“用分做单位”和 `Decimal` 两种可靠思路。需要注意：`round()` 适合展示或一般数值处理，不能单独解决所有金额精度问题；创建 `Decimal` 时应传字符串，例如 `Decimal("19.90")`，不要传已经产生二进制误差的浮点数。
