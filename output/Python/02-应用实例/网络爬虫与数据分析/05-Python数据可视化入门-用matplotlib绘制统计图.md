---
title: Python 数据可视化入门 05：用 matplotlib 绘制统计图
...
ax.plot(dates, views, marker='o', linewidth=2, label='浏览量')
...
fig, ax = plt.subplots(figsize=(8, 4))
ax.plot(dates, python_views, marker='o', label='Python')
ax.plot(dates, web_views, marker='s', label='前端')
...
fig, ax = plt.subplots(figsize=(8, 4))
bars = ax.barh(categories, article_counts)
...
word_counts = [800, 1200, 1500, 2200, 3000, 4500, 5200]
views = [90, 160, 210, 260, 430, 520, 610]

fig, ax = plt.subplots(figsize=(7, 4))
ax.scatter(word_counts, views, s=80, alpha=0.7)
...
fig, ax = plt.subplots(figsize=(7, 4))
ax.hist(views, bins=5, edgecolor='white')
...
| 图表 | 适合数据 |
| --- | --- |
| 柱状图 | 分类数据，比如 Python、前端、数据库 |
| 直方图 | 连续数字，比如浏览量、价格、耗时 |
...
python_views = [120, 150, 180, 220, 260, 300, 1200]
web_views = [90, 110, 160, 200, 240, 260, 310]

fig, ax = plt.subplots(figsize=(7, 4))
ax.boxplot([python_views, web_views], tick_labels=['Python', '前端'])
...
fig, ax = plt.subplots(figsize=(7, 4))
ax.plot(dates, views, marker='o', label='浏览量')

ax.set_title('每日文章浏览量')
ax.set_xlabel('日期')
ax.set_ylabel('浏览量')
ax.legend()
...
| --- | --- |
| `ax.set_title()` | 设置图表标题 |
| `ax.set_xlabel()` | 设置横轴标题 |
| `ax.set_ylabel()` | 设置纵轴标题 |
| `ax.legend()` | 显示图例 |
...
categories = ['Python', '前端', '数据库', '运维']
counts = [18, 24, 9, 7]
colors = ['#4f81bd', '#9bbb59', '#f79646', '#8064a2']
...
max_views = max(views)
max_index = views.index(max_views)
...
ax.annotate(
    f'最高：{max_views}',
    xy=(dates[max_index], max_views),
    xytext=(dates[max_index], max_views + 30),
...
fig.tight_layout()
fig.savefig(output_dir / 'views_trend.png', dpi=150, bbox_inches='tight')
plt.close(fig)
```
...
axes[0].plot(dates, views, marker='o')
axes[0].set_title('浏览量趋势')
axes[0].set_xlabel('日期')
axes[0].set_ylabel('浏览量')
axes[0].grid(True, linestyle='--', alpha=0.4)

axes[1].bar(dates, comments)
...
def draw_category_views(summary_df, output_dir):
    sorted_df = summary_df.sort_values('total_views', ascending=False)
...
def main():
    ...
    summary_df = build_category_summary()
    trend_df = build_daily_trend()

    draw_category_views(summary_df, output_dir)
    draw_category_share(summary_df, output_dir)
    draw_daily_trend(trend_df, output_dir)

    print(f'图表已保存到：{output_dir.resolve()}')
...
df = pd.read_csv('article_summary.csv')
...
1. 画图前用 `pd.to_numeric()` 确认数字列可以计算。
2. 保存后用 `plt.close(fig)` 释放资源。

不要直接相信 CSV 里的字段类型。

爬虫数据、导入数据、人工编辑过的表格都可能混入空值或字符串。
...
`fig.autofmt_xdate()` 会自动调整日期标签，减少重叠。
...
`matplotlib.use('Agg')` 要写在 `import matplotlib.pyplot as plt` 之前。
...
CHART_DPI = 150
CHART_SIZE = (8, 4.5)
CHART_COLORS = ['#4f81bd', '#9bbb59', '#f79646', '#8064a2']
