...
- 除非用户在当前请求中明确要求浏览器验证，或明确授权本轮使用浏览器，否则禁止主动启动浏览器、打开页面、截图或执行 DOM 交互；“前端测试/调试”本身不等于获得浏览器授权。
- 文案、样式微调、明确的条件判断、字段映射、接口字段名、后台脚本、文档和权限判定等改动，不做浏览器回归。
- 只有在用户明确要求或授权后，且问题确实依赖真实 DOM、布局、事件链路或浏览器控制台、现有构建测试和日志无法覆盖时，才允许使用浏览器；同一问题经过多次修改并由用户反馈仍未解决，只能作为建议用户授权的理由，不能替代授权。
