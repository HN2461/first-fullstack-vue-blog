...
- 优先用构建、单元测试、接口测试、日志和静态检查确认结果，浏览器只用于这些手段无法覆盖的交互和渲染问题。
- 后端接口、数据结构、脚本、文档、权限判定这类改动，不要为了“习惯性确认”反复打开浏览器。
- 前端只有在涉及页面布局、动效、表单交互、路由跳转、权限可见性、实际渲染结果时，才安排浏览器验证。
- 浏览器验证要一次性合并多个检查点，尽量复用同一个会话和同一组页面，避免为每个细节单独启动新轮次。
- 截图、日志、录屏只在确有排查价值时保留；验证结束后应清理临时产物，避免占用磁盘空间。

前端视觉/交互改动还应按需打开浏览器检查：
...
如果只是文案、纯逻辑、接口字段名、后台脚本或其他不影响最终渲染的改动，优先不做重复浏览器回归。
