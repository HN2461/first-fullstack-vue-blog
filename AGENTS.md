...
- Vue / JS 代码风格：
  ...
  - 不使用分号。
- 代码拆分硬约束：
  - 代码文件行数统一控制在 450–500 行区间：低于 450 行为正常，超过 450 行必须优先拆分，代码文件不得超过 500 行。
  - Vue 单文件组件、JS / TS / Java 等源码与工具文件均适用同一标准。
  - Markdown 文章、教程、项目文档、导出内容等非代码资料不受 500 行硬限制约束，但仍应保持结构清晰、便于维护。
