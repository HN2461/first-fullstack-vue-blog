...
export async function batchUpdateCategoryStatus(ids, status) {
  if (!Array.isArray(ids) || ids.length === 0) {
    throw createHttpError(400, 'CATEGORY_IDS_REQUIRED', '请选择要操作的分类')
  }

  if (!['active', 'hidden'].includes(status)) {
    throw createHttpError(400, 'INVALID_CATEGORY_STATUS', '分类状态不正确')
  }

  const result = await Category.updateMany(
    { _id: { $in: ids }, isSystem: { $ne: true } },
    { $set: { status } }
  )
...
export async function batchDeleteCategories(ids) {
  if (!Array.isArray(ids) || ids.length === 0) {
    throw createHttpError(400, 'CATEGORY_IDS_REQUIRED', '请选择要删除的分类')
  }
