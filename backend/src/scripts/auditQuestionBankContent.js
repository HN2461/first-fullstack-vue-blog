...
const leafCategoryKeys = new Set(categories
  .filter((item) => !categories.some((candidate) => candidate.parentKey === item.key))
  .map((item) => item.key))
const papers = JSON.parse(fs.readFileSync(path.join(dataDir, 'papers.json'), 'utf8')).items
...
for (const question of questions) {
  ...
  if ((question.tags || []).some((tag) => !tag.trim() || tag.length > 30)) addIssue('error', question, '标签为空或超过 30 字字段限制')
  if ((question.tags || []).length > 12) addIssue('error', question, '标签超过 12 个字段限制')
  ...
  if (question.code?.startsWith('coverage-')) {
    if (!leafCategoryKeys.has(question.categoryKey)) addIssue('error', question, '覆盖扩容题未放入叶子分类')
    if (question.type !== 'short_answer' || question.assessmentMode !== 'self') addIssue('error', question, '覆盖扩容题必须是提交后自评简答题')
    if (question.explanation.length < 500) addIssue('error', question, '覆盖扩容题解析少于 500 字')
    for (const heading of ['**答案与结论**', '**参考作答框架**', '**小白理解与核心原理**', '**项目实践与排错**', '**常见误区与面试追问**']) {
      if (!question.explanation.includes(heading)) addIssue('error', question, `覆盖扩容题解析缺少分段：${heading}`)
    }
  }
}
...
for (const key of leafCategoryKeys) {
  if (!questions.some((item) => item.code?.startsWith('coverage-') && item.categoryKey === key)) {
    issues.push({ level: 'error', code: '-', file: 'questions-coverage-expansion.json', message: `叶子分类缺少覆盖扩容题：${key}` })
  }
}
const paperKeys = papers.map((item) => item.key)
for (const paper of papers) {
  if (!paper.key || !paper.title) issues.push({ level: 'error', code: paper.key || '-', file: 'papers.json', message: '内置试卷缺少 key 或标题' })
  if (!paper.categoryKeys?.length || paper.categoryKeys.some((key) => !categoryKeys.has(key))) {
    issues.push({ level: 'error', code: paper.key || '-', file: 'papers.json', message: '内置试卷引用了不存在的分类' })
  }
  const pool = questions.filter((question) => paper.categoryKeys.some((key) => question.categoryKey === key || question.categoryKey.startsWith(`${key}.`)))
    .filter((question) => !paper.types?.length || paper.types.includes(question.type))
    .filter((question) => !paper.difficulties?.length || paper.difficulties.includes(question.difficulty))
  if (pool.length < paper.questionCount) {
    issues.push({ level: 'error', code: paper.key, file: 'papers.json', message: `抽题池只有 ${pool.length} 道，少于试卷要求 ${paper.questionCount} 道` })
  }
}
if (new Set(paperKeys).size !== paperKeys.length) issues.push({ level: 'error', code: '-', file: 'papers.json', message: '内置试卷 key 重复' })
...
console.log(`题库总数：${questions.length}；前端题：${frontend.length}；分类：${categories.length}；内置试卷：${papers.length}`)
