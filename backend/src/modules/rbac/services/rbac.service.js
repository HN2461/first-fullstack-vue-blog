...
const DEFAULT_MENUS = [
  ...
  { code: 'knowledge.ledger.monthly', name: '月表格', icon: 'CalendarOutlined', routePath: '/console/ledger/monthly', routeKey: 'knowledge.ledger.monthly', parentCode: 'knowledge.ledger', parentType: MENU_PARENT_TYPES.CHILD, sortOrder: 35, type: MENU_TYPES.SYSTEM },
  ...
  { code: 'questionbank.root', name: '题库', icon: 'ExperimentOutlined', routePath: '', routeKey: 'questionbank.root', parentType: MENU_PARENT_TYPES.ROOT, sortOrder: 17, type: MENU_TYPES.SYSTEM },
  { code: 'questionbank.overview', name: '题库总览', icon: 'DashboardOutlined', routePath: '/console/question-bank/overview', routeKey: 'questionbank.overview', parentCode: 'questionbank.root', parentType: MENU_PARENT_TYPES.CHILD, sortOrder: 10, type: MENU_TYPES.SYSTEM },
  { code: 'questionbank.questions', name: '题目库', icon: 'DatabaseOutlined', routePath: '/console/question-bank/questions', routeKey: 'questionbank.questions', parentCode: 'questionbank.root', parentType: MENU_PARENT_TYPES.CHILD, sortOrder: 20, type: MENU_TYPES.SYSTEM },
  { code: 'questionbank.papers', name: '试卷库', icon: 'FileDoneOutlined', routePath: '/console/question-bank/papers', routeKey: 'questionbank.papers', parentCode: 'questionbank.root', parentType: MENU_PARENT_TYPES.CHILD, sortOrder: 30, type: MENU_TYPES.SYSTEM },
  { code: 'questionbank.practice', name: '快速练习', icon: 'RocketOutlined', routePath: '/console/question-bank/practice', routeKey: 'questionbank.practice', parentCode: 'questionbank.root', parentType: MENU_PARENT_TYPES.CHILD, sortOrder: 40, type: MENU_TYPES.SYSTEM },
  { code: 'questionbank.review', name: '错题复习', icon: 'StarOutlined', routePath: '/console/question-bank/review', routeKey: 'questionbank.review', parentCode: 'questionbank.root', parentType: MENU_PARENT_TYPES.CHILD, sortOrder: 50, type: MENU_TYPES.SYSTEM },
  { code: 'questionbank.attempts', name: '作答记录', icon: 'ClockCircleOutlined', routePath: '/console/question-bank/attempts', routeKey: 'questionbank.attempts', parentCode: 'questionbank.root', parentType: MENU_PARENT_TYPES.CHILD, sortOrder: 60, type: MENU_TYPES.SYSTEM },
...
async function runRbacSeed(forceBuiltinSync = false) {
  ...
  if (mediaShareMenuCreated && mediaMenu && mediaShareMenu) {
    // 新菜单上线时继承一次原媒体权限，避免已有管理员升级后突然失去分享管理能力。
    await Role.updateMany(
      {
        $and: [
          { menuIds: mediaMenu._id },
          { menuIds: { $ne: mediaShareMenu._id } }
        ]
      },
      { $addToSet: { menuIds: mediaShareMenu._id } }
    )
  }
  ...
  if (knowledgeRootMenu && todoMenu) {
    await Role.updateMany(
      {
        $and: [
          { menuIds: knowledgeRootMenu._id },
          { menuIds: { $ne: todoMenu._id } }
        ]
      },
